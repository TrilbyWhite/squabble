...
void draw_init(const char *fname) {
	...
	ctile = cairo_create(imgBrd);
	/* green table */
	cairo_set_source_rgba(ctile,0.1,0.6,0.3,1);
	cairo_rectangle(ctile,0,0,1250,950);
	cairo_fill(ctile);
	/* off white backing */
	cairo_set_source_rgba(ctile,0.92,0.92,0.85,1);
	cairo_rectangle(ctile,250,100,750,750);
	cairo_fill(ctile);
	/* bonus shades */
	int i, j;
	for (i = 0; i < 15; i++) for (j = 0; j < 15; j++) {
		if (board[i][j].word_bonus == 3)
			cairo_set_source_rgba(ctile,1,0,0,0.5);
		else if (board[i][j].word_bonus == 2)
			cairo_set_source_rgba(ctile,1,0,0,0.15);
		else if (board[i][j].tile_bonus == 3)
			cairo_set_source_rgba(ctile,0,0,1,0.5);
		else if (board[i][j].tile_bonus == 2)
			cairo_set_source_rgba(ctile,0,0,1,0.15);
		else
			cairo_set_source_rgba(ctile,1,1,1,0);
		cairo_rectangle(ctile,250+i*50,100+j*50,50,50);
		cairo_fill_preserve(ctile);
		cairo_set_source_rgba(ctile,0,0,0,1);
		cairo_stroke(ctile);
	}
